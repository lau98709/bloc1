...
Librairie pour le format GIFT

Contient les fonctions nécessaires pour interprêter les fichiers au format GIFT
et l'enregistrement au format GIFT et JSON.

L'ensemble des fonctions sont intégrées à la classe d'objet "Question"

Remarque : pour le format JSON, si on l'enregistre dans un fichier séparé, 
le fichier HTML qui le lit n'est pas exploitable sans serveur web.
Donc, on enregistre le code JSON à l'intérieur d'un HTML. Le résultat est
exploitable directement sur une machine sans serveur. Mais, cela crée une
faille, les réponses seront intégrées au code source HTML...
...
separators = {'=','~'}
charsp = ["\"", "?", "\'", "%", "\\"]       # Les caractères spéciaux JSON
...
def AddSlash( s ):
    # Rajouter un anti-slash "\" devant les caractères spéciaux
    so = ""
...
class Question():
    ...
    def WriteHTML(self, nomfichier, templatehtml):
        # Production d'un fichier html à partir d'un "template"
        # Dans le fichier "template", la balise %json% sera remplacé
        # par les données de la question au format JSON
        balise = "%json%"
        ...
        while True:
            ...
            if (c == balise[0]):        # si '%'
                ...
                while i < len(balise):
                    ...
                    if (len(c)==0): break
                    if (c != balise[i]):
                        # Si la balise finalement ne correspond pas
                        # alors écrire les caractères déjà lus
                        for j in range(0,i):
                            fout.write(balise[j])
                        fout.write(c)
                        break
                    i += 1
                if (i >= len(balise)):
                    # La balise %json% correspond, on le remplace
                    # par les données au format JSON
                    self.WriteJSON(fout)
            else:
                fout.write(c)
